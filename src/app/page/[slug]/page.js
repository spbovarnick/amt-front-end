...
export default function PagePage( ){
  ...
  const [pageDataIsFetching, setPageDataIsFetching] = useState(true);

  // fetch page data on mount
  useEffect(() => {
    (async () => {
      setPageDataIsFetching(true);
      const data = await fetchPageData(slug);
      setPageData(data);
      setFilters({ ...filters, pageTag: data.tag, pageCollection: data.collection });
      setPageDataIsFetching(false);
    })();
  ...
  useEffect(() => {
    if (!pageDataIsFetching) {
      (async () => {
        setAssocDataIsLoading(true)
        const data = await fetchAssociatedData(callFromPage, pageData.title)
        setLocations(data.locations)
        setPeople(data.people)
        setCollections(data.collections)
        setCommGroups(data.comm_groups)
        setTags(data.tags)
        setCarouselSlides(data.page_carousel_slides)
